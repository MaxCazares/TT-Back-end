...
class NodeConnection(threading.Thread):
    ...
    def decompress(self, compressed):
        """Decompresses the data given the type. It is used to provide compression to lower the network traffic in case of
           large data chunks."""
        self.main_node.debug_print(
            self.id + ":decompress:input: " + str(compressed))
        compressed = base64.b64decode(compressed)
        self.main_node.debug_print(
            self.id + ":decompress:b64decode: " + str(compressed))
        ...
        if isinstance(data, str):
            try:
                if compression == 'none':
                    ...
                    data = self.compress(data.encode(
                        encoding_type), compression)
                    if data != None:
                        self.sock.sendall(
                            data + self.COMPR_CHAR + self.EOT_CHAR)
            ...
            try:
                if compression == 'none':
                    ...
                    data = self.compress(json.dumps(
                        data).encode(encoding_type), compression)
                    if data != None:
                        self.sock.sendall(
                            data + self.COMPR_CHAR + self.EOT_CHAR)
        ...
        while not self.terminate_flag.is_set():
            ...
            if chunk != b'':
                ...
                while eot_pos > 0:
                    ...
                    aux = self.parse_packet(packet)
                    if isinstance(aux, dict) and 'result':
                        if 'result' in aux.keys():
                            self.current_message = aux

                    self.main_node.message_count_recv += 1
                    self.main_node.node_message(
